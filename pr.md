...
- No behavior changes for existing code unless a user enum already declares a conflicting `values()` with a different signature. Engine-registered internal methods cannot be redeclared; attempting to do so results in the usual "Cannot redeclare" compile error.
- Unit enums do not expose `values()` (consistent with design).
